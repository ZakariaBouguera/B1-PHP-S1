<?php
	...
	for ( $i = 0 ; count($liaisons)> $i ; $i+=1){
		list ( $nomsecteur, $codeliaisons,$distance,$portdepart,$portarrivee) = explode(":",$liaisons[$i]);
		if (! in_array($portarrivee, $codes)){
			array_push ($codes, $portarrivee);
	}
	}
	sort ($codes);
	echo " code des liaisons qui desservent le port des Quiberon : \n ";
	for ( $i = 0 ; count($codes)> $i ; $i+=1){
		echo  "\t - ", $codes[$i], "\n";
	}
